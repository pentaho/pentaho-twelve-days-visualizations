...
define([
    ...
    pentaho.viz.Chord = function( div ) {
        ...
        this.debug = false;
        if( this.debug ) console.log('pentaho.viz.Chord');
        ...
        this.highlight = function() {
            ...
            if( this.playing || this.highLightNo != -1 || !this.highLightsSet ) {
                ...
                if( this.chord ) {
                    if( showAll ) {
                        this.chord.style("fill", function(d) { return localThis.items[d.source.index].color })
                    } else {
                        this.chord.style("fill", function(d) {
                            return (d.source.index == localThis.highLightNo ||
                                ...
                                localThis.highlights[d.target.index] ) ? localThis.items[d.source.index].color : "rgba(255,255,255,0.1)"; })
                    }
                }
                ...
                var localThis = this;
                if( this.chord ) {
                    this.chord.style("fill", function(d) { return localThis.highlights[d.source.index] || localThis.highlights[d.target.index] ? localThis.items[d.source.index].color : "rgba(255,255,255,0.1)"; })
                }
            }
        };
        ...
        this.resize = function() {
            this.width = vizUtil.getClientWidth(this)-20,
            this.height = vizUtil.getClientHeight(this)-20,
        ...
        this.draw = function( dataTable, options ) {
            if( this.debug) console.log('pentaho.viz.Chord draw');
            ...
            // Add the group arc.
            var groupPath2 = this.group.append("path")
                .attr("id", function(d, i) { return "group" + i; })
                .attr("d", this.arc2)
                .style("fill", "none");
        ...
        this.init = function() {
            ...
            if( this.debug) console.log('pentaho.viz.Chord init()');
            ...
            this.titleStr = pentaho.common.Messages.getString('title');
        ...
        this.processData = function() {
            ...
            for( var idx=0; idx<rowsNos.length; idx++ ) {
                var values = this.dataTable.getDistinctFormattedValues(rowsNos[idx]);
                rowsDistinct[idx] = values.length;
            }
            for( var idx=0; idx<colsNos.length; idx++ ) {
                var values = this.dataTable.getDistinctFormattedValues(colsNos[idx]);
            ...
            for( var idx=0; idx<this.column2Names.length; idx++ ) {
                this.items.push( {
                    ...
                    color: pentaho.palettes[this.paletteNo].colors[idx],
                    cols: colsNos
                } );
